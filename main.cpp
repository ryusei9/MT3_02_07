...
const char kWindowTitle[] = "LE2B_13_サトウ_リュウセイ_MT3_02_06";
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	while (Novice::ProcessMessage() == 0) {
		...
		DrawAABB(aabb, worldViewProjectionMatrix, viewPortMatrix);
...
// ベクトルの各要素について最小値と最大値を計算するヘルパー関数
bool RayIntersectsAABB(const Vector3& segmentOrigin, const Vector3& rayDir, const AABB& aabb, float& tmin, float& tmax) {
	for (int i = 0; i < 3; i++) {
		float invDir = 1.0f / rayDir[i];
		float t0 = (aabb.min[i] - segmentOrigin[i]) * invDir;
		float t1 = (aabb.max[i] - segmentOrigin[i]) * invDir;
...
bool IsCollision(const AABB& aabb, const Segment& segment){
	Vector3 d = segment.diff - segment.origin; // 線分の方向ベクトル
	float tMin = 0.0f; // tminからtMinに変更
	float tMax = 1.0f; // tmaxからtMaxに変更

	// 各軸について判定する
	for (int i = 0; i < 3; i++) {
		if (std::abs(d[i]) < 1e-8) {
			// 線分がこの軸に平行な場合、線分の始点がボックスの範囲内にあるかを確認
			if (segment.origin[i] < aabb.min[i] || segment.origin[i] > aabb.max[i]) {
				return false;
			}
		} else {
			// 線分がこの軸に平行でない場合、交差範囲を求める
			float ood = 1.0f / d[i];
			float t1 = (aabb.min[i] - segment.origin[i]) * ood;
			float t2 = (aabb.max[i] - segment.origin[i]) * ood;
